...
export class ForStatement extends SlangNode {
    constructor(ast, collected, options) {
        super(ast, collected);
        this.kind = NonterminalKind.ForStatement;
        this.initialization = extractVariant(new ForStatementInitialization(ast.initialization, collected, options));
        this.condition = extractVariant(new ForStatementCondition(ast.condition, collected, options));
        if (ast.iterator) {
            this.iterator = extractVariant(new Expression(ast.iterator, collected, options));
        }
